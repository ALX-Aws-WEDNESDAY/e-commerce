...
class UserRegisterViewset(viewsets.ModelViewSet):
    ...
    def me(self, request):
        ...
        return Response(
            {"user": UserDetailSerializer(user).data, "roles": list(user_roles)},
            status=status.HTTP_200_OK,
        )
    ...
    def assign_role(self, request, pk=None):
        ...
        is_admin = Roles.objects.filter(
            user=current_user, role__iexact="admin"
        ).exists()
        ...
        role_obj, created = Roles.objects.update_or_create(
            user=user, defaults={"role": role.lower()}
        )
    ...
    def get_queryset(self):
        ...
        is_admin = Roles.objects.filter(user=user, role__iexact="admin").exists()
