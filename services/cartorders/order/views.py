...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        order_number = (
            f"ORD-{timezone.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:8].upper()}"
        )
    ...
    def cancel(self, request, pk=None):
        ...
        # Add tracking entry
        OrderTracking.objects.create(
            order=order, status="cancelled", message="Order cancelled by user"
        )
    ...
    def recent(self, request):
        ...
        recent_orders = Order.objects.filter(user=request.user).prefetch_related(
            "items"
        )[:5]
    ...
    def update_status(self, request, pk=None):
        ...
        # Add tracking entry
        tracking_message = (
            message or f"Order status changed from {old_status} to {new_status}"
        )
        OrderTracking.objects.create(
            order=order, status=new_status, message=tracking_message
        )
